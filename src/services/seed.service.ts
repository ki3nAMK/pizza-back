...
import { stores } from '@/seed/store.seed';
...
export class SeedService implements OnModuleInit {
  ...
  private async seedCategories() {
    for (const cat of dummyData.categories) {
      const exists = await this.categoryRepo.findOneByCondition({
        name: cat.name,
      });
      if (!exists) {
  ...
  private async seedCustomizations() {
    for (const cust of dummyData.customizations) {
      const exists = await this.customizationRepo.findOneByCondition({
        name: cust.name,
      });
      if (!exists) {
  ...
  private async seedStores() {
    for (const store of stores) {
      const exists = await this.storeRepo.findOneByCondition({
        name: store.name,
      });
      if (!exists) {
  ...
  private async seedMenus() {
    for (const menu of dummyData.menu) {
      const exists = await this.menuRepo.findOneByCondition({
        name: menu.name,
      });
      if (exists) continue;

      // Tìm category
      const category = await this.categoryRepo.findOneByCondition({
      ...
      for (const custName of menu.customizations) {
        const cust = await this.customizationRepo.findOneByCondition({
          name: custName,
        });
